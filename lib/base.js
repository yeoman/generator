...
var Base = module.exports = function Base(args, options) {
  ...
  this.skipUpdate = this.options['skip-update'];

  if (_.isUndefined(this.skipUpdate)) {
    this.skipUpdate = options.argv && options.argv.original[0].indexOf(':') > -1;
  }
...
// Check for updates to the generator being used.
Base.prototype.checkForUpdate = function checkForUpdate() {
  var self = this;
  var next = this.async();
  var callback = {};
  var generatorPackage = self.pkg;

  this.once('error', function () {
    self.spawnCommand('yo', ['--skip-update', self.options.argv.original.join(' ')]);
  });

  var message = {
    performUpdate: 'An update is available. Would you like to upgrade now?',
    updateDenied: 'No problem! Carrying on...',
    upToDate: 'Your generator is up to date! Carrying on...'.cyan
  };

  // Helper function to log a message or emit an error before continuing.
  var escape = function (messageName, updateComplete) {
    if (!messageName) {
      next();
    } else if (message[messageName]) {
      // Log a message before continuing.
      console.log('\n' + message[messageName]);

      // `updateComplete` is only passed in after the user went through a
      // successful npm update. It is then passed to the `next` function, which
      // async flags as an "error", which the update event technically is. Since
      // the user has a fresh copy of the generator, we can't continue executing
      // the old generator's methods.
      next(updateComplete);
    } else {
      // Had an error :(
      self.emit('error', messageName);
      next(messageName);
    }
  };

  if (!generatorPackage) {
    generatorPackage = self.resolved.replace(/(\/.*generator[^\/]*)\/.*/, '$1/package.json');

    if (fs.existsSync(generatorPackage)) {
      generatorPackage = JSON.parse(fs.readFileSync(generatorPackage));
    } else {
      return escape();
    }
  }

  // Response from updateNotifier's check for available updates.
  callback.performUpdate = function (err, update) {
    if (err) {
      return escape(err);
    }
    ...
    self.prompt({
      type: 'confirm',
      name: 'performUpdate',
      message: message.performUpdate,
      default: true
    }, function (answers) {
      if (!answers.performUpdate) {
        return escape('updateDenied');
      }

      self.spawnCommand('npm', ['update', '-g', generatorPackage.name])
        .on('error', callback.updatePerformed)
        .on('exit', callback.updatePerformed);
    });
  };

  // Response from the `npm update`.
  callback.updatePerformed = function (err) {
    if (err) {
      return escape(err);
    }

    escape('upToDate', 'updateComplete');
  };

  updateNotifier({
    packageName: generatorPackage.name,
    packageVersion: generatorPackage.version,
    callback: callback.performUpdate
  });
};
...
Base.prototype.run = function run(args, cb) {
  ...
  if (!self.skipUpdate) {
    methods.unshift('checkForUpdate');
  }
