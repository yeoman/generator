...
conflicter.add = function add(conflict) {
  ...
  if(!conflict.file) throw new Error('Missing conflict.file option');
  if(!conflict.content) throw new Error('Missing conflict.content option');
...
conflicter.collision = function collision(filepath, content, cb) {
  ...
  // todo: handle non utf8 file (images, etc.) and compare mtime instead,
  // something like that.
