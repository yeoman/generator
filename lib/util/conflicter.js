...
conflicter.collision = function collision(filepath, content, cb) {
  ...
  if(!grunt.file.exists(filepath)) {
    grunt.log.write('create'.green).writeln(' ' + filepath);
    return cb(null, 'create');
  }

  var actual = grunt.file.read(path.resolve(filepath));
  if(actual === content) {
    grunt.log.write('identical'.blue).writeln(' ' + filepath);
    return cb(null, 'identical');
  }

  if(self.force) {
    grunt.log.write('force'.yellow).writeln(' ' + filepath);
  ...
  (function ask() {
    grunt.helper('prompt', [{
      message: 'Overwrite ? (enter "h" for help) [Ynaqdh]?',
      name: 'overwrite',
      default: 'h'
      ...
      var answer = results.overwrite;

      var ok = 'Ynaqdh'.split('').filter(function(valid) {
        return valid === answer;
      }).length;
      ...
      if(answer === 'n') {
        grunt.log.write('skip'.yellow).writeln(' ' + filepath);
        return cb(null, 'skip');
      }
