process.logging = process.logging || require('./util/log');
...
generators.test = require('./test/helpers');
...
//
// - grunt  - Grunt instance from which cli.tasks and cli.options are used.
// - args   - When provided, overrides parsed arguments from grunt.
// - opts   - When provided, overides parsed options from grunt.
//
// Returns the resolved generator, or undefined with output to the console of
// the general help.
generators.init = function init(grunt, args, opts) {
  if(!generators.options) {
  ...
  if(args) {
    generators.name = args.shift();
  }

  // cli arguments (nopt's remains)
  args = args || generators.args;
  // options (nopt's hash option)
  opts = opts || generators.options;

  // line break the first run
  console.log();
...
generators.help = function help(args, options, config) {
  var users = generators.lookupHelp(process.cwd(), args, options, config),
  ...
  var groups = {};
  namespaces.sort().forEach(function(namespace) {
  ...
  var out = [
    'Usage: yeoman init GENERATOR [args] [options]',
    '',
    'General options:',
...
generators.invoke = function invoke(namespace, args, options, config, cb) {
  ...
  log.emit('up');
  log.invoke(namespace.replace(/^yeoman:/, ''));
  log.emit('up');

  generator.on('end', log.emit.bind(log, 'down'));
  generator.on('end', log.emit.bind(log, 'down'));
...
generators.create = function create(namespace, args, options, gruntConfig) {
  ...
  generator._hooks = generator._hooks.map(function(hook) {
    var config = generator.fallbacks,
...
generators.lookupHelp = function lookupHelp(basedir, args, options, config) {
  ...
  found = found.filter(function(generator) {
    return typeof generator.module === 'function';
  }).sort(function(a, b) {
...
//
// **Depracated**. To be removed once yeoman's master repo has been updated.
//
