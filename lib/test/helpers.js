...
helpers.mockPrompt = function (generator, answers) {
  ...
  generator.prompt = function (prompts, done) {
    origPrompt(prompts, done);

    var answerPrompt = function (prompt) {
      var answer = answers[prompt.name];
      answer.split('').forEach(function (character) {
        process.stdin.emit('keypress', character, {
          name: character,
          sequence: character
        });
      });
    ...
    if (Array.isArray(prompts)) {
      prompts.forEach(answerPrompt);
    } else {
      answerPrompt(prompts);
    }
  };
