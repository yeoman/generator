...
var Environment = module.exports = function Environment(args, opts) {
  ...
  // register builtin (currently, only generator generator)
  this.register(path.join(__dirname, './generators/generator'), 'generator');
};
...
Environment.prototype.namespace = function namespace(filepath) {
  ...
  ns = ns
    .replace(new RegExp(sep + '?node_modules' + sep + '?'), '')
    .replace(new RegExp(sep + '(index|main)$'), '')
