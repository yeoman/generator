...
//
// It provides high-level API to create and run generators, as well as further
...
Environment.prototype.register = function register(name, namespace) {
  ...
  if(typeof name === 'string') {
    ...
    generator.namespace = this.namespace(path.relative(this.cwd, generator.resolved));
  } else {
    generator = name;
  }
...
Environment.prototype.alias = function alias(match, value) {
  ...
  return aliases.reduce(function(res, alias) {
    if(!alias.match.test(res)) { return res; }
...
Environment.prototype.namespace = function namespace(filepath) {
  if(!filepath) throw new Error('Missing namespace');
  ...
  // cleanup prefix
  // XXX consider caching the regexp
  ns = ns.replace(new RegExp('^(' + this._prefixes.join('|') + ')'), '');
...
Environment.prototype.remote = function remote(name, done) {
  ...
  npm.on('exit', function(code) {
    ...
    log.info('You should see aditional generators available').write()
