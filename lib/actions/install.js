 ...
 * @param {Boolean} [sync = false] whether to run sync or to schedule the install.
 */
...
install.scheduleInstallTask = function(
  ...
  if (typeof paths === 'boolean') {
    ...
    sync = options;
    options = {};
  } else if (typeof spawnOptions === 'boolean') {
    sync = spawnOptions;
    spawnOptions = {};
  }
  ...
  if (sync) {
    return this.spawnCommandSync(installer, args, spawnOptions);
  }
 ...
 * @param {Boolean} [options.sync] - whether to run sync or schedule the install.
 */
install.installDependencies = function(options) {
  ...
  if (options.npm !== false) {
    ...
    this.npmInstall(null, getOptions(options.npm), null, options.sync);
  }

  if (options.yarn) {
    msg.commands.push('yarn install');
    this.yarnInstall(null, getOptions(options.yarn), null, options.sync);
  }

  if (options.bower !== false) {
    msg.commands.push('bower install');
    this.bowerInstall(null, getOptions(options.bower), null, options.sync);
  }
 ...
 * @param {Boolean} [sync] whether to run sync or schedule the install.
 */
install.bowerInstall = function(cmpnt, options, spawnOptions, sync) {
  return this.scheduleInstallTask('bower', cmpnt, options, spawnOptions, sync);
};
 ...
 * @param {Boolean} [sync] whether to run sync or schedule the install.
 */
install.npmInstall = function(pkgs, options, spawnOptions, sync) {
  return this.scheduleInstallTask('npm', pkgs, options, spawnOptions, sync);
};
 ...
 * @param {Boolean} [sync] whether to run sync or schedule the install.
 */
install.yarnInstall = function(pkgs, options, spawnOptions, sync) {
  return this.scheduleInstallTask('yarn', pkgs, options, spawnOptions, sync);
};
