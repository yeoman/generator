...
/**
 * Combine package manager cmd line arguments.
 *
...
install.scheduleInstall = function (installer, paths, options, cb) {
  ...
  }.bind(this), { once: installer + ' ' + args.join(' ') });
...
/*
 * Receives a list of `components`, and an `options` object to schedule through the bower installation.
 *
...
install.bowerInstall = function install(cmpnt, options, cb) {
  var result = this.bowerSchedule(cmpnt, options, cb);
 ...
 * @returns {Object}
 */
install.bowerSchedule = function schedule(cmpnt, options, cb) {
  return this.scheduleInstall('bower', cmpnt, options, cb);
};

/*
 * Receives a list of `components`, and an `options` object to schedule through the npm installation.
 *
...
install.npmInstall = function install(pkgs, options, cb) {
  var result = this.npmSchedule(pkgs, options, cb);
  this.env.runLoop.run();
  return result;
};
 ...
 * @param {Function} [cb]
 */
install.npmSchedule = function schedule(pkgs, options, cb) {
  return this.scheduleInstall('npm', pkgs, options, cb);
};
