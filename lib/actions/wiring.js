...
wiring.generateBlock = function generateBlock(blockType, optimizedPath, filesBlock, searchPath, indent) {
  var blockStart, blockEnd;
  var blockSearchPath = '';

  if (searchPath !== undefined && searchPath !== false) {
  ...
  indent = parseInt(Math.abs(indent), 10);
  indent = new Array(isNaN(indent) ? 9 : indent * 2 + 1).join(' ');

  blockStart = '\n' + indent + '<!-- build:' + blockType + blockSearchPath + ' ' + optimizedPath + ' -->\n';
  blockEnd = indent + '<!-- endbuild -->\n';
...
wiring.appendFiles = function appendFiles(htmlOrOptions, fileType, optimizedPath, sourceFileList, attrs, searchPath, indent) {
  ...
  if (fileType === 'js') {
    sourceFileList.forEach(function (el) {
      files += indent + '<script ' + attrs + ' src="' + el + '"></script>\n';
    });
    blocks = this.generateBlock('js', optimizedPath, files, searchPath, origIndent);
    updatedContent = this.append(html, 'body', blocks);
  } else if (fileType === 'css') {
    sourceFileList.forEach(function (el) {
      files += indent + '<link ' + attrs + ' rel="stylesheet" href="' + el  + '">\n';
    });
    blocks = this.generateBlock('css', optimizedPath, files, searchPath, origIndent);
