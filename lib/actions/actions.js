...
actions._prepCopy = function _prepCopy(source, destination, process) {
  ...
  if (typeof process === 'function' && !binary) {
    ...
    var processed = process(body, source, destination, {
    ...
    if(_.isObject(processed)) {
      ...
      var values = _.pick(processed, Object.keys(copy))

      if(_.isObject(values.encoding)) {

        values.encoding = values.encoding.encoding || 'utf8';

      }

      copy = _.extend(copy, values);
