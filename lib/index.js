...
class Generator extends EventEmitter {
  ...
  constructor(args, options) {
    ...
    if (!this.env.adapter || !this.env.runLoop || !this.env.sharedFs || !this.env.fs) {
      throw new Error(
    ...
    if (!this.options.skipCheckEnv) {
      this.checkEnvironmentVersion('3.0.0-beta0');
    }
  }
      ...
      continueQueue => {
        ...
        runAsync(function() {
          ...
          .catch(err => {
            ...
            delete self.runningState;
            try {
              self.env.emit('error', err);
            } catch (error) {
              console.log('Fatal: Environment must have a error listener');
              process.exit(1);
            }
          });
      },
  ...
  queueBasicTasks() {
    ...
    this.env.runLoop.add('conflicts', done => {
      this.env.conflicter.resolve(err => {
        if (err) {
  ...
  _writeFiles(done) {
    ...
    const conflictChecker = through.obj(function(file, enc, cb) {
      ...
      self.env.conflicter.checkForCollision(file, (err, status) => {
        if (err) {
          cb(err);
          return;
        }
