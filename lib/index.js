...
class Generator extends EventEmitter {
  constructor(args, options) {
    ...
    // Initialize inheritedPriorities
    this.inheritedPriorities = {};
  }
  ...
  /**
   * Add prototypes to be queue with the main generator.
   * This method will look only for named priorities.
   *
   * @param {object} [...Prototype] Prototypes to be queued.
   * @param {boolean}     [cleanup] If false, the current queued prototypes will be removed.
   */
  inheritPriorities(...prototypes) {
    const cleanup = typeof _.last(prototypes) === 'boolean' ? prototypes.pop() : false;
    this.inheritedPriorities = cleanup ? {} : this.inheritedPriorities;

    const self = this;
    prototypes.forEach(prototype => {
      this.env.runLoop.queueNames.forEach(queue => {
        let property = Object.getOwnPropertyDescriptor(prototype, queue);
        if (property) {
          Object.defineProperty(self.inheritedPriorities, queue, property);
        }
      });
    });
  }
  ...
  run(cb) {
    const promise = new Promise((resolve, reject) => {
      ...
      methods.push(...Object.getOwnPropertyNames(this.inheritedPriorities));
      ...
      function addInQueue(name) {
        let property = Object.getOwnPropertyDescriptor(Object.getPrototypeOf(self), name);
        if (!property) {
          property = Object.getOwnPropertyDescriptor(self.inheritedPriorities, name);
          debug(`Queueing inherited priority ${name}`);
        }
