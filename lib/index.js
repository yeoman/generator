...
class Generator extends EventEmitter {
  ...
  constructor(args, options) {
    ...
    if (Array.isArray(this.options.customPriorities)) {
      const customPriorities = this.options.customPriorities.map(
        (customPriority) => {
   ...
   */
  rootGeneratorName() {
    const {pkg} = readPkgUp.sync({cwd: this.resolved});
    return pkg ? pkg.name : '*';
  }
   ...
   */
  _writeFiles(done) {
    const conflictChecker = this.env.conflicter.createChecker();
    this.fs.commit(this._transformStreams.concat([conflictChecker]), () => {
      done();
    });
  }
}
