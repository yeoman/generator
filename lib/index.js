...
class Generator extends EventEmitter {
  ...
  constructor(args, options) {
    ...
    if (Array.isArray(this.options.customPriorities)) {
      ...
      const conflicting = customPriorities.find(queue =>
        Generator.queues.includes(queue.name)
      );
      if (conflicting) {
        throw new Error(
          `Cannot add a custom queue with same name of built-in queue ${conflicting.name}`
        );
      }
