...
import { type Task, type BaseFeatures, type TaskOptions, type BaseOptions, type Priority } from '../types.js';
...
export default function tasksMixin<Parent extends Constructor<BaseGenerator>>(parent: Parent) {
  return class TasksMixin extends parent {
    _composedWith: any[] = [];
     ...
     */
    queueMethod(method: Task['method'], methodName: string, queueName: Task['queueName'], reject: Task['reject']);
    queueMethod(method: Record<string, Task['method']>, methodName: string | Task['reject'], reject?: Task['reject']);
    queueMethod(
     ...
     */
    async composeWith(generator: string | string[] | { Generator: any; path: string }, immediately?: boolean);
    async composeWith(
    ...
    async composeWith(
      generator: string | string[] | { Generator: any; path: string },
      args?: string[] | (BaseOptions & { arguments?: string[]; args?: string[] }) | boolean,
      ...
      const instantiate = async (generatorFactory, path) => {
        generatorFactory.resolved = path;
        generatorFactory.namespace = this.env.namespace(path);

        return this.env.instantiate(generatorFactory, parsedArgs, parsedOptions as any);
      };
      ...
      if (typeof generator === 'string') {
        try {
          ...
          instantiatedGenerator = await this.env.create(generator, parsedArgs, parsedOptions);
        }
      } else {
...
await this.composeWith({
  ...
  ...
     ...
     */
    queueTransformStream(transformStreams: Transform | Transform[]): this {
